            ...
            <div class="row-fluid">
                ...
                <div class="block">
                    <div class="navbar navbar-inner block-header" style="text-align:center">
                        <div class="muted pull-left">QUẢN LÍ ADMIN</div>
                    </div>
                            ...
                            <div class="table-toolbar">
                                <div>
                                    <div style="float:left;">
                                        <a href="/Admin/TMAdmin"><button class="btn btn-success">Thêm mới <i class="icon-plus icon-white"></i></button></a>
                                    </div>
                            ...
                            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered"
                                ...
                                <tbody id="myTable">
                                    <c:forEach var="nguoiDung" items="${requestScope.nguoiDungs}">
                                        <tr class="odd gradeX">
                                            <td><input type="checkbox" name="chkKichHoat" class="form-control" <c:if test="${nguoiDung.kichHoat}">checked</c:if> /></td>
                            ...
                            <div class="dataTables_paginate paging_bootstrap pagination" style="text-align:center">
                                <ul>
                                    <c:if test="${param.page != 1}">
                                        <li class="prev"><a href="/Admin/NguoiDungAdmin?page=${param.page - 1}">← Previous</a></li>
                                    </c:if>
                                    <c:forEach var="j" begin="1" end="${totalPage}">
                                        <c:if test="${param.page == j}">
                                            <li class="active"><a href="/Admin/NguoiDungAdmin?page=${j}">${j}</a></li>
                                        </c:if>
                                        <c:if test="${param.page != j}">
                                            <li><a href="/Admin/NguoiDungAdmin?page=${j}">${j}</a></li>
                                        </c:if>
                                    </c:forEach>

                                    <c:if test="${param.page < totalPage}">
                                        <li class="next"><a href="/Admin/NguoiDungAdmin?page=${param.page + 1}">Next → </a></li>
                                    </c:if>
                                </ul>
                            </div>
                        </div>
